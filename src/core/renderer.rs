...
impl DrawCommand {
    fn draw_sprite_params(params: &mut SpriteParams, camera: &Camera, width: u32, height: u32) {
        let cam_scale = camera.scale;
        let cam_pos = camera.get_pos();
        if params.relative_to_cam  {
            let half_width = Vector2::new(width as f32 / 2.0, height as f32 / 2.0);
    ...
    pub fn draw(self, canvas: &mut WindowCanvas, asset_m: &AssetManager, renderer: &mut Renderer) {
        match self {
            DrawCommand::Sprite(mut params) => {
                ...
                Self::draw_sprite_params(&mut params, &renderer.camera, width, height);
                ...
                let dest_rect = Rect::new(
                    pos.x.floor() as i32,         
                    pos.y.floor() as i32,
                    (width as f32 * scale_x).floor() as u32, // scale
                    (height as f32 * scale_y).floor() as u32 // scale
                );
