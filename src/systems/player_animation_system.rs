...
pub fn player_animation_init() -> StartFn {
    Box::new(|ecs: &mut ECS, _renderer: &mut Renderer| {
        for (sprite, anim_player, _p_tag) in 
...
pub fn player_animation_update() -> UpdateFn {
    Box::new(|ecs: &mut ECS, _delta_time: f32| {
        for (p_input, p_data, sprite, anim_player, vel) in 
            ecs.query_comp::<(&PlayerInput, &PlayerData, &mut Sprite, &mut AnimationPlayer, &Velocity)>() {
